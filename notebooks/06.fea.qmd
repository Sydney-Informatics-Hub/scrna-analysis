...
1. Load in the pseudobulked dataset and differential expression data from the previous notebook
...
The following block assumes you have run all the notebooks up to and including `05.pseudo.qmd`. That notebook produces a `.Rds` file called `outputs/differential_expression_analyses.Rds` that will be read in now. We will also read in the p-value and fold-change thresholds defined in that notebook.
...
de_thresholds <- readRDS(here("inputs/de_thresholds.csv"))
