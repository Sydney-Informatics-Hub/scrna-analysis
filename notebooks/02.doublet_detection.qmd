...
```{r inputs}
samples <- read_tsv(here("inputs/doublet_samples.tsv"))
cluster_resolutions <- read_tsv(here("inputs/cluster_resolutions.tsv"))
...
if(remove_doublets) {
  all_doublets <- lapply(all_doublets, function(s) {
    ...
    s <- FindClusters(s, resolution = res, verbose = 0)
