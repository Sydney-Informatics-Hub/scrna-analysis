...
**Please be aware** that in this process, some genes may not successfully convert to Ensembl IDs and will be lost; as such, we highly recommend that you re-run `nf-core/scrnaseq` for all samples if possible.
...
all_seurat_objects <- lapply(transpose(samples), function(x) {
  ...
  have_ens_ids <- "gene_versions" %in% colnames(s@assays$RNA@meta.data) && all(startsWith(s@assays$RNA@meta.data$gene_versions, "ENS"))
  have_gene_symbols <- "gene_symbols" %in% colnames(s@assays$RNA@meta.data) && !all(startsWith(s@assays$RNA@meta.data$gene_symbols, "ENS"))
  if(!have_ens_ids) {
...
bad_cluster_degs <- lapply(all_init_sct, function(s) {
  ...
  if(all(startsWith(rownames(degs_combined), "ENS"))) {
    ...
    degs_combined$ensembl_id <- rownames(s@assays$RNA)
  } else {
    degs_combined$gene_symbol <- rownames(s@assays$RNA)
