...
if(using_ens_ids) {
  ens2gene <- AnnotationDbi::mapIds(
    EnsDb.Hsapiens.v86::EnsDb.Hsapiens.v86,
    keys = rownames(integrated_rna_norm@assays$RNA),
    column = "SYMBOL",
    keytype = "GENEID"
  )
  stopifnot(all(rownames(integrated_rna_norm@assays$RNA) == names(ens2gene)))
  keep <- !is.na(ens2gene)
  ens2gene <- ens2gene[keep]
  integrated_rna_norm <- integrated_rna_norm[keep,]
  rownames(integrated_rna_norm@assays$RNA) <- ens2gene
}
