...
de$Gene.Symbol <- gene2ens$gene_symbols[match(de$Gene, gene2ens$gene_versions)]
...
```{r show_de_genes}
de_sig <- de %>% filter(sig == "sig")
for(n in unique(de_sig$Identity)) {
  de_sig %>%
    filter(Identity == n) %>%
    dplyr::select(Gene.Symbol, avg_log2FC, pct.1, pct.2, p_val, p_val_adj_all) %>%
...
```{r gene_expression_lists}
genes_to_plot <- list(
  bcells = c("CD79A", "IGHD", "IGHV3-33"),
  tcells = c("CD3D", "CD3E", "CD247"),
  b_t = c("TCF4", "SYK")
)
```
...
rn <- rownames(integrated_rna_norm@assays$RNA)
...
p_ge <- lapply(names(genes_to_plot), function(x) {
  ...
  exp_df %>%
    filter(comparison_group %in% cmp_grps) %>%
