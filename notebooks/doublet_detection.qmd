...
In the following block, provide a path to a CSV sample sheet defining your sample names, the cluster resolution to use, and (optionally) the expected multiplet rate for each sample. The column names should be `sample` for the sample name column, `res` for cluster resolution, and `multiplet_rate` for the multiplet rates. Cluster resolution for each sample should have been determined while running the QC notebook. Multiplet rates should be the proportion of each sample expected to be multiplets; for example, if 5% of the cells were expected to be multiplets, the value should be `0.05`. The sample names will be used to find the filtered RDS files saved by the QC notebook, which will be named like `outputs/<SAMPLE_NAME>.filtered_clustered.Rds`. An example of the samplesheet is given below; the sample `normal_sample` has been specified with a multiplet rate of `0.076`, while the other sample `tumour_sample` has had this column left blank, and so the automatic multiplet rate estimation method will be used:

```
sample,res,multiplet_rate
normal_sample,1.0,0.076
tumour_sample,1.2,
```
...
```{r inputs}
samples <- read_csv("data/doublet_samples.csv")
