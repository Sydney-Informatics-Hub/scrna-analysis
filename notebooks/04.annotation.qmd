...
**NOTE:** On first use, the function `celldex::HumanPrimaryCellAtlasData()` may create a cache in your home directory at `~/.cache/R/AnnotationHub`. This is a one-time setup step, but be aware that if you are on a HPC system such as gadi it may cause issues if your home directory is near quota. Please ensure that you have at least 1GB of free space available in your home directory.

---

**❱❱❱ OPTIONAL ACTION ❰❰❰**

The following block sets the lower threshold of the number of cells required to keep a given annotation. If a particular cell type annotation has been assigned to this many or fewer cells, that cell type will be discarded and the cell will be labelled as "Other".

By default, we have set this to `10`. If you are studying rare cell types, you may want to set variable `min_cell_annot_threshold` in the next block to a lower value, or even `0` to keep all HPCA annotations.

---

```{r set_min_cell_threshold_for_annotation}
min_cell_annot_threshold <- 10
```

```{r annotate_cell_types_hpca, eval = FALSE}
# Create cache dir if it doesn't already exist
hpca_cache_dir <- "~/.cache/R/AnnotationHub"
if(!dir.exists(hpca_cache_dir)) {
  dir.create(hpca_cache_dir, recursive = TRUE)
}

if(using_ens_ids) {
  ref <- celldex::HumanPrimaryCellAtlasData(ensembl = TRUE)
} else {
  ref <- celldex::HumanPrimaryCellAtlasData(ensembl = FALSE)
}
...
keep <- table(predicted$labels) > min_cell_annot_threshold
...
if(run_custom_annotation && using_ens_ids) {
...
scores <- integrated@meta.data %>%
  dplyr::select(Cluster = all_of(cluster_name), all_of(custom_programs)) %>%
  pivot_longer(
