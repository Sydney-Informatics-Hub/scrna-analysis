...
```{r inputs}
samples_file <- "data/integration_samples.txt"

# Load data
sample_names <- scan(samples_file, character())
...
To define the final cluster resolution to use, provide a text file with just the resolution value on the first line. For example:
...
cluster_res <- scan(cluster_res_file, character())
cat(paste0("Cluster resolution: ", cluster_res, "\n"))
```
...
for (s in all_doublets) {
  ...
  SaveSeuratRds(s, paste0("outputs/", sample_name, ".filtered_clustered.doublets_detected.integrated.Rds"))
}
```
